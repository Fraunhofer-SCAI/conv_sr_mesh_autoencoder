...
def read_plymesh(path):    
    data_mesh = read_ply(path)
    x_np = np.array(data_mesh['points'])
    face_np = np.array(data_mesh['mesh'])
    
    #face = torch.from_numpy(face_np).T.type(torch.long)    
    #x = torch.tensor(x_np.astype('float32'))
