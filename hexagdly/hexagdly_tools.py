...
def plot_hextensor(
    ...
    for i in range(nimages):
        ...
        #[ -2.6 * np.shape(tensor[image_range[0] + a, channel_range[0] + b])[1] - 1,  1, ]
        ax.set_ylim(
            [ -1.1 * np.shape(tensor[image_range[0] + a, channel_range[0] + b])[0] - 1,  1, ]
        )
        ax.set_axis_off()
        if 'eps' in figsave:
            p = PatchCollection(
                np.array(hexagons[i]), cmap=cmap, edgecolors="k", linewidth=1
            ) 
        else:
            p = PatchCollection(
